    ...
    (err) => {
      ...
      return connection.query(
        ...
        (err2, changes) => {
          ...
          const changedUser = changes[0];
          const { password, ...user } = changedUser; // In the quest they ask us to return the changed object, I hide the password but erasing password in property and it will appear in the result.
          const host = req.get("host");
          const location = `http://${host}${req.url}/${user.id}`;
